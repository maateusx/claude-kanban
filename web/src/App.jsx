                ...
                <DragOverlay>
                  {activeId ? (
                    <CardBody task={tasks.find(t => t.id === activeId)} queue={queue}
                      defaultModel={project.defaultModel} pending={pending} dragging />
                  ) : null}
                </DragOverlay>
              </DndContext>
              {detail && (
...
function Column({ col, tasks, queue, onRun, onOpen, onAddTask, selectedId, defaultModel, pending, sort, onSort, onArchiveAll }) {
      ...
      <div className="flex-1 space-y-3 overflow-y-auto px-3 pb-4">
        {ordered.map(t => (
          <Card key={t.id} task={t} queue={queue} onRun={onRun} onOpen={onOpen}
            selected={t.id === selectedId} defaultModel={defaultModel} pending={pending} />
        ))}
        {tasks.length === 0 && (
          onAddTask ? (
            <button onClick={onAddTask} title={`Nova task em ${col.label}`}
...
function CardBody({ task, queue, onRun, onOpen, selected, defaultModel, pending = [], innerRef, handleProps, hidden, dragging }) {
