...
export const api = {
  ...
  cleanup: pid => fetch(`/api/projects/${pid}/cleanup`).then(j),
  applyCleanup: (pid, sel) => fetch(`/api/projects/${pid}/cleanup`, opts(sel)).then(j),
  discardTask: (pid, tid) => fetch(`/api/projects/${pid}/tasks/${tid}/discard`, opts({})).then(j),
  plugins: pid => fetch(`/api/projects/${pid}/plugins`).then(j),
  setPlugins: (pid, enabled) => fetch(`/api/projects/${pid}/plugins`, { ...opts({ enabled }), method: 'PUT' }).then(j),
  ...
  analyze: (pid, types, question, report) => fetch(`/api/projects/${pid}/analyze`, opts({ types, question, report })).then(j),
  ...
  searchSources: pid => fetch(`/api/projects/${pid}/search-sources`).then(j),
  addSearchSource: (pid, source) => fetch(`/api/projects/${pid}/search-sources`, opts(source)).then(j),
  patchSearchSource: (pid, sid, patch) => fetch(`/api/projects/${pid}/search-sources/${sid}`, { ...opts(patch), method: 'PATCH' }).then(j),
  removeSearchSource: (pid, sid) => fetch(`/api/projects/${pid}/search-sources/${sid}`, { method: 'DELETE' }).then(j),
  fetchSearchSource: (pid, sid) => fetch(`/api/projects/${pid}/search-sources/${sid}/fetch`, opts({})).then(j),
  fetchAllSearchSources: pid => fetch(`/api/projects/${pid}/search-sources/fetch-all`, opts({})).then(j),
  importSearchItems: (pid, items) => fetch(`/api/projects/${pid}/search-sources/import`, opts({ items })).then(j),
