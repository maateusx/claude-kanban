...
| Método | Path | Body | Resposta |
| --- | --- | --- | --- |
| `GET` | `/api/projects/:projectId/pending-actions` | — | `{ actions: [{ id: "pa-xxxxxx", timestamp, label, command, taskId, status, policy }] }` — parse de `.claude/claude-kanban/pending-actions.md`. `policy`: `null`, ou `{ pattern, result, output }` quando a ação foi executada e resolvida pela `guardrailPolicy` (`result` = `"exit N"` ou `"erro: …"`). |
...
| `GET` | `/api/run/queue` | — | `queueView`: `{ actives: [{ projectId, taskId }], queue: [{ projectId, taskId }], merges: [{ projectId, taskId, branch, into, kind: "base" | "parent", stage: null | "verify" }], maxConcurrency }`. `merges` é a fila de merge (ver FUNCIONALIDADES 17h-2); o primeiro item é o que está sendo processado. |
...
| `goal.attention` | `{ projectId, taskId, reason }` | O loop de lacunas parou sem cumprir a spec (teto de rodadas, teto de custo ou lacunas repetidas); `reason` lista o que falta. Vira o webhook `goal_needs_human`. |
| `task.attention` | `{ projectId, taskId, reason }` | O diagnóstico automático (`autopilot.diagnose`) classificou a falha como externa (ou sem dependência identificável); a task segue `blocked`. Vira o webhook `task_needs_human`. |
...
| `run.queue` | `queueView` | A fila foi alterada por fora do fluxo normal (remoção de um projeto, cancelamento de um item da fila) ou a fila de merge andou. |
